...
"""
Description:
input sentence is segmented list and keywords are also segmented which could be applied to 
sentence for other preprocess such as transfer word to id and rule id
"""
...
#             self.non_word_boundaries = set(string.digits + string.ascii_letters + '_')
	...
	def extract_keywords(self, sentence, span_info=False):
		...
		current_dict = self.keyword_trie_dict
		# print("keyword trie dict", current_dict)
			...
			if char not in self.non_word_boundaries:
				...
				if self._keyword in current_dict or char in current_dict:
					...
					if longest_sequence_found:
						# print("==longest_sequence_found==", longest_sequence_found)
						keywords_extracted.append((longest_sequence_found, sequence_start_pos, idx+1))
		...
		if span_info:
			return keywords_extracted
		return keywords_extracted #[value[0] for value in keywords_extracted]
